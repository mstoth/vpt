...
"""
If windows, we have a twain module to import. None for Apple or Linux yet. 
This means you can only scan from Virtual Page Turner on Windows.  Other OS
you need to use the scanner software available on that system. 
"""
...
VPT_HOME must be set for proper execution
"""
...
if vpthome:
    ...
    wx.MessageBox('Setting vpthome to current directory.')
    vpthome=os.curdir

XferByFile='File'
XferNatively='Natively'
recognizedExtensions = ['gif','bmp','tiff','jpeg','jpg','JPG','png']
...
# Menu Handling Routines
        ...
        # self.pdf.Bind(wx.EVT_CHAR, self.OnKeyDown)
        # self.pdf.Bind(wx.EVT_KEY_DOWN, self.OnKeyDown)
        # self.pdf.Bind(wx.EVT_KEY_UP, self.OnKeyDown)
        # self.pdf.setLayoutMode('TwoColumnLeft')
        # self.pdf.Hide()
        self.pdfON = False
        ...
        if not os.path.exists(optionsFile):
            ipnl = InitPanel()
            dlg = ipnl.dlg
            ipnl.musicDir.SetPath(vpthome)
            if (ipnl.dlg.ShowModal() == wx.ID_OK):
                musicDir = ipnl.musicDir.GetPath()
        ...
        self.enhanceOptions = []
        self.revertPage = None
        self.revertPageName = None
...
# Tool Bar Creation
    def createToolBar(self):
        ...
        self.toolbar.SetToolBitmapSize((105,105))
    ...
    def toolbarData(self):
        return(("Revert",vpthome + "/images/revertbig.png","Revert Image", self.OnRevert),
               ("","","",""),
               ("Blur",vpthome + "/images/blurbig.png","Blur Image", self.OnBlur),
               ("Rotate Clockwise", vpthome + "/images/rotatebig.png","Rotate Image Clockwise", self.OnRotate),
               ("Rotate Counter Clockwise", vpthome + "/images/rotatembig.png","Rotate Image Counter Clockwise", self.OnRotatem),
               ("Black Threshold", vpthome + "/images/blacktbig.png","Black Threshold", self.OnBlackThreshold),
               ("Sharpen", vpthome + "/images/sharpenbig.png","Sharpen Image",self.OnSharpen),
               ("Convert All", vpthome + "/images/convertallbig.png","Convert All Pages", self.OnConvertAll),
               ("Revert All", vpthome + "/images/revertallbig.png","Revert All Pages", self.OnRevertAll),
               ("Save",vpthome + "/images/savebig.png","Save Conversion",self.OnSaveEnhancement))
...
# Tool Bar Methods    
    ...
    def ConvertAll(self):
        if self.revertPageName:
            pgm.currentPiece.GetCurrentPage().SetFileName(self.revertPageName)
        self.revertPage = None
        piece = pgm.GetCurrentPiece()
        npages = piece.NumberOfPages()
        fdir = piece.GetName()
        for i in range(0,npages):
            piece.SetCurrentPage(i)
            # pgs.SetValue(int(i*100/npages)+ 10)
            # frm.Refresh(True)
            page = piece.GetCurrentPage()
            fname = page.GetFileName()
            # self.LoadCurrentPiece()
            self.statusText = 'Converting ' + fname + '  of ' + str(npages) + ' pages '
            self.SetStatusText(self.statusText)
            self.Refresh(True)
            wx.SafeYield()
            self.EnhanceImage()
            self.SaveEnhancement()
        # frm.Destroy()


    def OnSharpen(self,event):
        self.enhanceOptions.append(' -sharpen 2x2 ')
        self.SetStatusText(''.join(self.enhanceOptions))
        self.EnhanceImage()

    def OnBlackThreshold(self,event):
        dlg = wx.TextEntryDialog(None,"Enter Threshold (<65536)","Black Threshold","40000",style=wx.OK|wx.CANCEL)
        if dlg.ShowModal() == wx.ID_OK:
            self.enhanceOptions.append(' -black-threshold ' + dlg.GetValue() + ' -background white ')
            self.SetStatusText(''.join(self.enhanceOptions))
            self.EnhanceImage()

    def OnRotate(self,event):
        dlg = wx.TextEntryDialog(None,"Enter Angle","Angle","1",style=wx.OK|wx.CANCEL)
        if dlg.ShowModal() == wx.ID_OK:
            self.enhanceOptions.append(' -rotate ' + dlg.GetValue() + ' -fill white -background white')
            self.SetStatusText(''.join(self.enhanceOptions))
            self.EnhanceImage()

    def OnRotatem(self,event):
        dlg = wx.TextEntryDialog(None,"Enter Angle","Angle","-1",style=wx.OK|wx.CANCEL)
        if dlg.ShowModal() == wx.ID_OK:
            self.enhanceOptions.append(' -rotate ' + dlg.GetValue() + ' -fill white ')
            self.SetStatusText(''.join(self.enhanceOptions))
            self.EnhanceImage()

    def OnBlur(self,event):
        self.enhanceOptions.append(' -blur 2 ')
        self.SetStatusText(''.join(self.enhanceOptions))
        self.EnhanceImage()

    def OnSaveEnhancement(self,event):
        self.SaveEnhancement()
        self.enhanceOptions = []

    def SaveEnhancement(self):
        fdir = pgm.currentPiece.GetName()
        page = pgm.currentPiece.GetCurrentPage()
        fname = self.revertPageName
        src = fdir + os.sep + 'temp.gif'
        if os.path.exists(src):
            dest = fdir + os.sep + fname
            if os.path.exists(dest):
                os.remove(dest)
                os.rename(src,dest)
                pgm.currentPiece.GetCurrentPage().SetFileName(fname)
                self.revertPage = None
                self.revertPageName = None
                self.enhanceOptions = []
                self.LoadCurrentPiece()
    ...
    def EnhanceImage(self):
        if not self.revertPage:
            self.revertPage = pgm.currentPiece.GetCurrentPage()
            self.revertPageName = self.revertPage.GetFileName()
        page = self.revertPage
        fdir = pgm.currentPiece.GetName()
        fname = self.revertPageName
        src =   fdir + os.sep + fname        
        coptions = ""
        for o in self.enhanceOptions:
            coptions = coptions + o
        # self.enhanceOptions = []
        dest = fdir + os.sep + 'temp.gif'
        # self.run('convert',src+coptions+dest)
        cmd = 'convert "' + src + '"' + coptions + '"' + dest + '"'
        if wx.Platform == '__WXMSW__':
            cvt = self.whereis('convert.exe')
        else:
            cvt = self.whereis('convert')
        self.SetStatusText('Processing...' + src + coptions)
        if cvt != "":
            os.system(cmd)
            page.SetFileName('temp.gif')
            self.LoadCurrentPiece()
        else:
            wx.MessageBox('Can not find convert program.')
            # r = os.spawnv(os.P_WAIT,cvt,('convert','"'+src+'"',coptions,'"'+dest+'"'))
        # file = os.popen(cmd)
        # result = file.read()
        # file.close()        
        # r = os.system(cmd)
    ...
    def whereis(self,pgm):
        for path in string.split(os.environ["PATH"],os.pathsep):
            if os.path.exists(path+os.sep+pgm):
                return path+os.sep+pgm
        return ""
    ...
    def run(self, program, *args):
        ...
        for path in string.split(os.environ["PATH"], os.pathsep):
            ...
            try:
                rv = os.spawnv(os.P_WAIT, file, (file,) + args)
                return rv
            except os.error:
                pass
    ...
    def OnRevertAll(self,event):
        if self.revertPageName:
            pgm.currentPiece.GetCurrentPage().SetFileName(self.revertPageName)
        fdir = pgm.currentPiece.GetName()
        src = fdir + os.sep + 'original' + os.sep + '*.gif'
        dest = fdir
        if wx.Platform == '__WXMSW__':
            cmd = 'copy "' + src + '" "' + dest + '"'
        else:
...
# Menu Creation        
    ...
    def MenuItemData(self,type=None):
        ...
        if type == '&Edit':
            return[['&Add Annotation...',self.OnAnnotate],
                   ...
                   ['&Convert PDF File...',self.OnConvertPDF],
                   ['&Toggle Enhance Mode',self.OnEnhance]]
        if type == '&View':
    ...
    def OnConvertPDF(self,event):
        dlg = wx.FileDialog(self,message = 'Select PDF File')
        if (dlg.ShowModal() == wx.ID_OK):
            path = dlg.GetPath()
            dlg = wx.DirDialog(self, message = 'Select Directory for Conversion')
            if (dlg.ShowModal() == wx.ID_OK):
                dest = dlg.GetPath()
                cmd = 'gswin32c -sDEVICE=jpeg -dBATCH -dNOPAUSE -r200 -sOutputFile=' + dest + '\\Page%03d.jpg ' + path
                # cmd = 'convert ' + path + ' ' + dest + '/Page%03d.gif'
                self.SetStatusText('Converting...')
                os.system(cmd)
                self.SetStatusText('Done')
                wx.MessageBox('PDF File is Converted.')

    def OnEnhance(self,event):
        if not self.toolbar:
            self.createToolBar()
            self.toolbar.Realize()
        if not self.toolBarOn:
            self.toolBarOn = True
            self.toolbar.Show()
            if wx.Platform == '__WXMSW__':
                self.ypos = self.ypos+105
        else:
            self.revertPage = None
            self.toolBarOn = False
            self.toolbar.Hide()
        ...
        page = pgm.currentPiece.GetCurrentPage()
        fname = page.GetFileName()
        if not os.path.exists(fdir + os.sep + 'original'):
            os.mkdir(fdir + os.sep + 'original')
            if (wx.Platform == '__WXMSW__'):
                os.system('copy "' + fdir + os.sep + '*.gif" "' + fdir + os.sep + 'original"')
            else:
                os.system('cp "' + fdir + os.sep + '*.gif" "' + fdir + os.sep + 'original"')
...
# Event Handlers
    ...
    def GoToPiece(self):
        ...
        if (pgm.SetPieceByName(choice) == 0):
        	self.SetStatusText(''.join(["Can't find ", choice]))
	else:
        	self.SetStatusText(''.join(['Loading ', choice]))
        self.LoadCurrentPiece()
        dlg.Destroy()
    ...
    def OnAbout(self,event):
        ...
        info.WebSite = ("http://www.virtualpianist.com/Software/Software3.htm",
...
# General Methods
    ...
    def LoadCurrentPiece(self):
        ...
        if self.currentPiece:
            ...
            if self.currentPage:
                pageType = self.currentPage.GetFileType()
                if (pageType != 'pdf'):
                    self.pdfON = False
                    if self.pdf:
                        self.pdf.Hide()
                    pageName = self.currentPage.GetFileName()
                    self.Bind(wx.EVT_PAINT, self.OnPaint)
                    fileName = ''.join([self.pieceName,os.sep,pageName])
                    ...
                    if self.pdf:
                        ...
                        self.pdf.setView('Fit')
                        self.pdf.setShowScrollbars(False)
                        if self.VIEWMODE == 'Fit Width':
                            self.pdf.setLayoutMode('Single Page')
                        else:
                            self.pdf.setLayoutMode('TwoColumnLeft')
